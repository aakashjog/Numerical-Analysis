...
\begin{question}
	Let $f \in C^4$.
	Using Taylor expansion, find an approximation of $f''(a)$ of the form
	\begin{align*}
	...
	\begin{enumerate}
		\item
			Choose $A$, $B$, $C$, such that $m$ is maximized.
		\item
			Assume that for some $M > 0$,
			\begin{align*}
				\left| f''''(x) \right| & \le M
			\end{align*}
			Assume that the value s of $f$ are known up to an error bounded by $\varepsilon$.
			The Differentiation formula above is calculated using the noisy data.
			Bound the overall error in the approximation of $f''(a)$.
		\item
			What is the optimal $h$ for the error bound?
	\end{enumerate}
\end{question}
...
\begin{solution}
	\begin{enumerate}[leftmargin=*]
		\item
			...
			Therefore,
			\begin{align*}
				\frac{f(a + h) - 2 f(a) + f(a + h)}{h^2} & = \quad \frac{f(a) - 2 f(a) + f(a)}{h^2}                                \\
                                                                         & \quad + \frac{-h f'(a) - 0 + h f'(a)}{h^2}                              \\
                                                                         & \quad + f''(a)                                                          \\
                                                                         & \quad \frac{\frac{h^3}{6} f'''(a) - \frac{h^3}{6} f'''(a)}{h^2}         \\
                                                                         & \quad \frac{\frac{h^4}{24} f''''(c_1) + \frac{h^4}{24} f''''(c_2)}{h^2} \\
                                                                         & = f''(a) + \frac{2 h^2}{24} \frac{f''''(c_1) + f''''(c_2)}{2}
			\end{align*}
			Therefore, by the intermediate value theorem, for $c \in [c_1,c_2]$,
			\begin{align*}
				\frac{f''''(c_1) + f''''(c_2)}{2} & = f''''(c)
			\end{align*}
			Therefore,
			\begin{align*}
				\frac{f(a + h) - 2 f(a) + f(a + h)}{h^2} & = f''(a) + \frac{h^2}{12} f''''(c)
			\end{align*}
			Therefore,
			\begin{align*}
				e & = \frac{h^2}{12} f''''(c)
...
\begin{question}
	Find an integration rule of the form
	\begin{align*}
		\int\limits_{0}^{h} f(x) \dif x & \approx A f(0) + B f'(0) + C f(h)
	\end{align*}
	where $f \in C^3$.
	Find the error formula, and find a composite integration method based on the integration rule, for $\int\limits_{a}^{b} f(x) \dif x$.\\
	How many sample points are needed to guarantee an error of absolute value less than or equal to $\frac{10^{-9}}{72}$, with $f(x) = \sin x$, and $[a,b] = [0,1]$.
\end{question}
